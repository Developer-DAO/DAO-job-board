...
CREATE TABLE public.jobs (
  id UUID PRIMARY KEY NOT NULL DEFAULT gen_random_uuid(),
  organization_id UUID NOT NULL REFERENCES public.organizations (id),
  jobtitle VARCHAR NOT NULL CHECK (jobtitle <> ''),
  jobdescription TEXT NOT NULL CHECK (jobdescription <> ''),
  jobposition VARCHAR CHECK (jobposition <> ''),
  jobtype VARCHAR CHECK (jobtype <> ''),
  jobcompensation VARCHAR CHECK (jobcompensation <> ''),
  jobmax VARCHAR CHECK (jobmax <> ''),
  jobmin VARCHAR CHECK (jobmin <> ''),
  jobequity VARCHAR CHECK (jobequity <> ''),
  joblocation VARCHAR CHECK (joblocation <> ''),
  jobcontact VARCHAR CHECK (jobcontact <> ''),
